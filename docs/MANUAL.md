...
refl : ∀ (A : Type (?ℓ734)) (a : A) -> Eq {?ℓ734} A a a
```
...
proving idPoly : ∀ (A : Type (?ℓ818)) -> A -> A
...
proving lift : Type (?ℓ832) -> Type (?ℓ833)
...
```
thena spine> :choices
1064  intro   untried: solve, regret, prove
```
...
thena spine> retry
retrying 1064: solve
backtracking to 1064: regret
thena spine> :show
▶ let ? id : ∀ (A : Type₀) -> A -> A in
  id
thena spine> :choices
1064  regret   untried: prove
```
...
stuck: already at the root
  undoing that would backtrack to 722, which was chosen before this line — retry 722 to take it
```
